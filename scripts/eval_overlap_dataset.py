...
def cal_pr_curve(pair_dist, ground_truth_file_name, thre_range=[0,1], interval=0.01):
    ...
    for thres in np.arange(thre_range[0], thre_range[1]+0.01, interval):
        tps = 0
        fps = 0
        tns = 0
        fns = 0
        ...
        for idx in range(101,len(ground_truth)-1):
            gt_idxes = np.array(ground_truth[int(idx)])
            reject_flag = False
...
def plotPRC(precisions, recalls, F1, recall_list, print_2file=False, snapshot=None, dataset='kitti'):
    ...
    save_name = '/home/zuosibo/SGLC_ws（fujian）/out/PRC_%s.png'%(dataset)
...
def main():
    ...
    parser.add_argument('--dataset', type=str, default='ford',
                        help='kitti or kitti360 ')
    parser.add_argument('--gt_file', type=str, default='/home/zuosibo/NewDisk/loop_data/loop_detection/overlap_based/ford/loop_gt_seq00_0.3overlap_inactive.npz',
                        help='the ground truth .txt file ')
    parser.add_argument('--pred_file', type=str, default='/home/zuosibo/SGLC_ws（fujian）/out/ford_campus/00_overlap_pairs_veri_.txt',
                        help='the prebdicted  .txt file ')
    # parser.add_argument('--dataset', type=str, default='kitti',
    #                     help='kitti or kitti360 ')
    # parser.add_argument('--gt_file', type=str, default='/home/zuosibo/NewDisk/data/kitti/loop_data/loop_detection/overlap_based/kitti/loop_gt_seq00_0.3overlap_inactive.npz',
    #                     help='the ground truth .txt file ')
    # parser.add_argument('--pred_file', type=str, default='/home/zuosibo/SGLC_ws（fujian）/out/kitti/00_overlap_pairs_veri.txt',
    #                     help='the prebdicted  .txt file ')
