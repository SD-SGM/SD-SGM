...
#ifndef MYPROJECT_NDT_H
#define MYPROJECT_NDT_H
...
public:
    ...
    std::vector<int> order_vec = {0, 22, 23, 24, 21, 20, 0, 0, 0, 10, 11, 12, 13, 15, 16, 14, 17, 9, 18, 19, 0, 0, 0, 0, 0, 0};
    struct NDTCell
    {
    ...
    struct GridCell {
        ...
        void addShape(int shape) {
            ...
            if (counts[shape - 1] > count_max && counts[shape - 1] >= double(total_counts) / 8.0) {  // 如果出现次数超过了之前最大值并且超过总次数的25% ？12.5%
...
public:
    NDT(float resolution_);
    Eigen::MatrixXd readpc(const std::vector<float> & cloud, const std::vector<uint32_t> & label, int num);   //读取点云文件，保存至
    ...
    static std::pair<double, int> distanceBtnNDTScanContext(Eigen::MatrixXd& frame_sc, Eigen::MatrixXd& submap_sc);
    static std::pair<double, int> calculate_sim(Eigen::MatrixXd &desc1, Eigen::MatrixXd &desc2, int yaw);
