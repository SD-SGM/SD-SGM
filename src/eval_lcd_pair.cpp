...
int main(){
    ...
    auto sem_file1=data_cfg["eval_pair"]["label_file1"].as<std::string>();
    auto sem_file2=data_cfg["eval_pair"]["label_file2"].as<std::string>();
    ...
    auto score = SemGraph.loop_pairs_similarity(cloud_file1, sem_file1, cloud_file2, sem_file2); 
    auto end_time = std::chrono::steady_clock::now();
    auto duration = std::chrono::duration<double, std::milli>(end_time - start_time).count();

    std::cout <<score<<blue<< std::endl;
